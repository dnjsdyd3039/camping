					...
					<div class="p-r-45 p-r-0-lg">
						...
						<div class="p-t-32">
							...
							<%--  <div id="bobtn">
								...
								<button class="btn btn-outline-secondary m-2" href="boardModify?bocode=${boardView.bocode }" onclick="boardModify()">수정</button> 
								<button class="btn btn-outline-secondary m-2" href="boardDelete?bocode=${boardView.bocode }" onclick="boardDelete()">삭제</button>
								</c:if>	
							</div> --%>
						</div>
							...
							<div class="reply-item" id="replyList">
								<c:forEach items="${replyList }" var="reply">
									<div class="reply-item-text">
										...
										<c:if test="${reply.rpmid == sessionScope.loginId }">
					...
					.ajax({
						...
						success : function(result) {
							...
							for (var i = 0; i < result.length; i++) {
								...
								output += '<button type="button" class="btn btn-outline-success" id="' + result[i].rpcode + '" onclick="replyModifyInfo(' + result[i].rpcode + ')">수정</button>';
								output += '<button type="button" class="btn btn-outline-success" id="' + result[i].rpcode + '" onclick="replyDelete(' + result[i].rpcode + ',' + result[i].bocode + ')">삭제</button>';
								}
								output += '</div>';
								output += '</div>';
							}
